...
let clipboardWatcher: NodeJS.Timeout | undefined;
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const processClipboardCommand = vscode.commands.registerCommand(
        'golang-error-navigator.processClipboard',
        async () => {
    ...
    const toggleClipboardMonitoring = vscode.commands.registerCommand(
        'golang-error-navigator.toggleClipboardMonitoring',
        () => {
...
async function findExactTextInFile(filePath: string, searchText: string): Promise<ErrorLocation[]> {
 ...
 */
function generateSmartPatterns(text: string): string[] {
    ...
    if (noPunct !== cleanText) patterns.add(noPunct);
 ...
 */
function findBestMatch(searchText: string, line: string, patterns: string[]): { score: number, pattern: string } {
    const lowerLine = line.toLowerCase();
 ...
 */
function calculateSimilarity(searchText: string, pattern: string, line: string): number {
 ...
 */
function setupExternalTerminalSupport(context: vscode.ExtensionContext) {
 ...
 */
function isGoErrorInClipboard(text: string): boolean {
    // Clean up text
 ...
 */
async function processClipboardError(clipboardText: string) {
    ...
    const action = await vscode.window.showInformationMessage(
        `🔍 Found ${allMatches.length} potential matches for Go errors in clipboard!`,
        'Show Results',
        'Ignore'
    );
